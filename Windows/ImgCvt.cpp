...
void writeLargeImage(std::string path, int n) {
	char buf[MAX_PATH];

	HBITMAP hBmpDst = CreateBitmap(2400, 2400, 1, 32, imgLG);
	HBITMAP hBmpSrc = CreateBitmap(RESIZED_WIDTH, RESIZED_HEIGHT, 1, 32, renderImg);
	HDC hDcDst = CreateCompatibleDC(GetWindowDC(GetDesktopWindow()));
	HDC hDcSrc = CreateCompatibleDC(GetWindowDC(GetDesktopWindow()));
	SelectObject(hDcDst, hBmpDst);
	SelectObject(hDcSrc, hBmpSrc);
	SetStretchBltMode(hDcDst, HALFTONE);
	StretchBlt(hDcDst, 0, 0, 2400, 2400, hDcSrc, 0, 0, RESIZED_WIDTH, RESIZED_HEIGHT, SRCCOPY);
	int mx = -255;
	int mn = 300;
	int delta = 20;
	...
	if (0 == GetDIBits(hDcDst, hBmpDst, 0, 0, NULL, &MyBMInfo, DIB_RGB_COLORS)) {
		//cout << "error" << endl;
	}
	...
	if (0 == GetDIBits(hDcDst, hBmpDst, 0, MyBMInfo.bmiHeader.biHeight, (LPVOID)imgLG, &MyBMInfo, DIB_RGB_COLORS)) {
		//cout << "error2" << endl;
	}
	...
	for (int i = 0; i < 2400; i++) {
		for (int j = 0; j < 2400; j++) {
			UCHAR r = imgLG[i][j * 4];
			if (abs(r - mx) < delta + 1) imgLG[i][j * 4] = mx;
		}
	}
	sprintf(buf, "%s\\%s-%d 2400x2400.obj", path.c_str(), modelName, n);
	FILE *pFile = fopen(buf, "wb");
	UCHAR br = 0x00;
	int mainPixelNumber = 0;
	for (int i = 0; i < 2400; i++) {
		for (int j = 0; j < 2400; j++) {
			UCHAR r = imgLG[i][j * 4];
			if (r == 0x00) {
	...
	fclose(pFile);
	sprintf(buf, "%s\\%s-%d 2400x2400.bmp", path.c_str(), modelName, n);
	CreateBMPFile(buf, hBmpDst);
	DeleteDC(hDcDst);
	DeleteObject(hBmpDst);
	DeleteObject(hBmpSrc);
}

void writeSmallImage(std::string path, int n) {
	char buf[MAX_PATH];

	HBITMAP hBmpDst = CreateBitmap(120, 120, 1, 32, imgSM);
	HBITMAP hBmpSrc = CreateBitmap(RESIZED_WIDTH, RESIZED_HEIGHT, 1, 32, renderImg);
	HDC hDcDst = CreateCompatibleDC(GetWindowDC(GetDesktopWindow()));
	HDC hDcSrc = CreateCompatibleDC(GetWindowDC(GetDesktopWindow()));
	SelectObject(hDcDst, hBmpDst);
	SelectObject(hDcSrc, hBmpSrc);
	SetStretchBltMode(hDcDst, HALFTONE);
	StretchBlt(hDcDst, 0, 0, 120, 120, hDcSrc, 0, 0, RESIZED_WIDTH, RESIZED_HEIGHT, SRCCOPY);
	int mx = -255;
	int mn = 300;
	int delta = 20;
	...
	// get the actual bitmap buffer
	if (0 == GetDIBits(hDcDst, hBmpDst, 0, MyBMInfo.bmiHeader.biHeight, (LPVOID)imgSM, &MyBMInfo, DIB_RGB_COLORS)) {
		//cout << "error2" << endl;
	}
	...
	for (int i = 0; i < 120; i++) {
		for (int j = 0; j < 120; j++) {
			UCHAR r = imgSM[i][j * 4];
			if (abs(r - mx) < delta + 1) imgSM[i][j * 4] = mx;
		}
	}
	sprintf(buf, "%s\\%s-%d 120x120.obj", path.c_str(), modelName, n);
	FILE *pFile = fopen(buf, "wb");
	UCHAR br = 0x00;
	int mainPixelNumber = 0;
	for (int i = 0; i < 120; i++) {
	...
	fclose(pFile);
	sprintf(buf, "%s\\%s-%d 120x120.bmp", path.c_str(), modelName, n);
	CreateBMPFile(buf, hBmpDst);
	DeleteDC(hDcDst);
	DeleteObject(hBmpDst);
	DeleteObject(hBmpSrc);
